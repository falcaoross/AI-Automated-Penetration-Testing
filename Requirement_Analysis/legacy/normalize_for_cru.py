...
def load_doc_meta(structured_output_path: str) -> Dict[str, str]:
    ...
    if not p.exists():
        print(f"[WARNING]️  structured_output.json not found at {structured_output_path}")
...
def write_audit(
    ...
    status = "[OK] PASS" if passes else "[WARNING]️  NEEDS REVIEW"
    print(f"\n{'='*62}")
    ...
    if missing_page:
        print(f"  [WARNING]️  Missing page          : {missing_page}")
    if duplicate_ids:
...
def run(
    ...
    # -- Step 3: Normalize -----------------------------------------------------
    print(f"\n  Normalizing {len(requirements)} requirements...")
    normalized: List[Dict[str, Any]] = []
